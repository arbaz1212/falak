...
def redeem_command(message):
    ...
    if key in keys and key not in redeemed_keys:
        ...
        bot.send_message(chat_id, "𝗞𝗲𝘆 𝗿𝗲𝗱𝗲𝗲𝗺𝗲𝗱 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆!")
    else:
        if key in redeemed_keys:
            bot.send_message(chat_id, "𝗧𝗵𝗶𝘀 𝗸𝗲𝘆 𝗵𝗮𝘀 𝗮𝗹𝗿𝗲𝗮𝗱𝘆 𝗯𝗲𝗲𝗻 𝗿𝗲𝗱𝗲𝗲𝗺𝗲𝗱!")
        else:
...
def my_account(message):
    ...
    if found_user:
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            ...
            account_info = (f"𝗬𝗼𝘂𝗿 𝗔𝗰𝗰𝗼𝘂𝗻𝘁 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻 ❄️\n\n"
                            f"𝗨𝘀𝗲𝗿𝗻𝗮𝗺𝗲: {found_user.get('username', 'N/A')}\n"
                            f"𝗩𝗮𝗹𝗶𝗱 𝗨𝗻𝘁𝗶𝗹: {valid_until}\n"
                            f"𝗣𝗹𝗮𝗻: {found_user.get('plan', 'N/A')}\n"
                            f"𝗖𝘂𝗿𝗿𝗲𝗻𝘁 𝗧𝗶𝗺𝗲: {current_time}")
    else:
